...
Page({
  ...
  getOrderList(refresh){
    let that = this;
    console.log('this.data.totalPages:',this.data.totalPages);
    console.log('this.data.page:', this.data.page);


    if (!refresh && that.data.totalPages <= that.data.page - 1) {
      that.setData({
        nomore: true
      })
      return;
    ...
    util.request(api.OrderList, { page: that.data.page, size: that.data.size, order_status: that.data.orderId}).then(function (res) {
      if (res.errno === 0) {
        that.setData({
          orderList: refresh ? res.data.data:that.data.orderList.concat(res.data.data),
